...
/* small API to clean up data associated with a QEMU instance */

static void destroy_object(void *ptr)
{
    QOSGraphObject *obj = ptr;
    obj->destructor(obj);
}

static void qtest_queue_destroy(QOSGraphObject *obj, char *name)
{
    if (!obj->destructor) {
        printf("Warning: Node %s allocates memory but does not provide"
               " a destructor\n", name);
        return;
    }
    g_test_queue_destroy(destroy_object, obj);
}
...
static void run_one_test(const void *arg)
{
    ...
    obj = node->u.machine.constructor(global_qtest);
    qtest_queue_destroy(obj, node->name);
    alloc = get_machine_allocator(obj, node->name);
    ...
    for (;;) {
        ...
        case QEDGE_CONSUMED_BY:
            ...
            obj = node->u.driver.constructor(obj, alloc, edge_arg);
            qtest_queue_destroy(obj, node->name);
            break;
    ...
    node->u.test.function(obj, test_arg, alloc);
}
