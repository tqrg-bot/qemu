...
static inline CCPrepare gen_prepare_cc_slow(DisasContext *s, int jcc_op, TCGv reg)
{
    switch(jcc_op) {
    case JCC_O:
    ...
    case JCC_BE:
        gen_compute_eflags(s);
        return (CCPrepare) { .cond = TCG_COND_NE, .reg = cpu_cc_src, .mask = 0x41 };
    case JCC_S:
    ...
    case JCC_L:
        ...
        return (CCPrepare) { .cond = TCG_COND_NE, .reg = reg, .mask = 0x80 };
    default:
    case JCC_LE:
        gen_compute_eflags(s);
        if (reg == cpu_cc_src) {
        ...
        tcg_gen_shri_tl(reg, cpu_cc_src, 4); /* CC_O -> CC_S */
        tcg_gen_xor_tl(reg, reg, cpu_cc_src);
        return (CCPrepare) { .cond = TCG_COND_NE, .reg = reg, .mask = 0xc0 };
    }
}

/* perform a conditional store into register 'reg' according to jump opcode
   value 'b'. In the fast case, T0 is guaranted not to be used. */
static inline CCPrepare gen_prepare_cc(DisasContext *s, int b, TCGv reg)
{
...
static void gen_op_cmov_reg_v(DisasContext *s, int ot, int b, int reg, int t0)
{
    ...
    cc = gen_prepare_cc(s, b, cpu_T[0]);
    if (cc.cond == TCG_COND_NEVER) {
        return;
    }
    if (cc.cond == TCG_COND_ALWAYS) {
    ...
    tcg_gen_movcond_tl(cc.cond, cpu_regs[reg], cc.reg, cc.reg2, t0, cpu_regs[reg]);
#ifdef TARGET_X86_64
    if (ot == OT_LONG) {
