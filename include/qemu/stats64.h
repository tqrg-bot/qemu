...
#if __SIZEOF_LONG__ < 8 && !defined(__i386__)
...
static inline void stat64_set(const Stat64 *s, uint64_t value)
{
...
static inline void stat64_add(Stat64 *s, uint64_t value)
{
    ...
    for (;;) {
        ...
        if (result < low) {
            /* If there is a carry, take the lock.  */
            if (stat64_add32_carry(s, low)) {
                return;
            }
            continue;
        }
...
static inline void stat64_min(Stat64 *s, uint64_t value)
{
    ...
    do {
        ...
        if (orig_high == high) {
            ...
            /* See if we were lucky; read again high.  Pairs with the barrier
             * in stat64_lock.
             */
            smp_rmb();
            orig_high = atomic_read(&s->high);
